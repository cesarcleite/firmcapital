...
function getTooltipHtml(type) {
  const tooltips = {
    ...
    vantagem: {
      ...
      formula: "(Ganho FII - Ganho Direto) / Ganho Direto × 100",
      interp: "150% = FII ganhou 150% a mais. -30% = Direto ganhou 30% a mais."
    },
    diferenca: {
...
function updateCharts() {
  ...
  if (ctxComparativo) {
    // Calcular valores finais para comparação
...
function updateOtherCharts(commonOptions) {
  ...
  if (ctxCustos && custosFII.length > 0) {
    ...
    // Definir todos os datasets disponíveis
    const allDatasets = [
      {
        id: "admin",
        label: "Taxa Admin",
      ...
      },
      {
        id: "itbi",
        label: "ITBI",
        data: primeiros60.map((c) => c.itbi || 0),
    ...
    // Armazenar datasets para filtros
    window.allCostDatasets = allDatasets;
    window.primeiros60Custos = primeiros60;
    
    charts.custos = new Chart(ctxCustos, {
      type: "bar",
      data: {
        labels: primeiros60.map((c) => `Mês ${c.mes}`),
        datasets: allDatasets, // Todos visíveis por padrão
      },
      options: {
        responsive: true,
        maintainAspectRatio: false,
