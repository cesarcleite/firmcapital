...
function setupCurrencyInputs() {
  ...
  percentInputs.forEach((input) => {
      ...
      // Format initial value
      if (input.value) {
          // Replace dot with comma if present (e.g. from 4.50 to 4,50)
          if (input.value.indexOf('.') !== -1) {
      ...
      input.addEventListener("change", function() {
          // Parse value: replace comma with dot for calculation
          let valStr = this.value.replace(',', '.');
          let val = parseFloat(valStr);
          
          if (!isNaN(val)) {
              // Format back to comma: 4.5 -> "4,50"
...
function updateInterface(
  ...
  const vantagem =
    Math.abs(valorTotalDireto) > 0
      ? (diferenca / Math.abs(valorTotalDireto)) * 100
      : 0;
  ...
  try {
    ...
    const resultados = {
      investimentoDireto: {
        ...
        dividendosTotal: totalDividendosDireto,
        patrimonioFinal: plFinalDireto,
      },
      fii: {
        valorTotal: valorTotalFII,
        dividendosTotal: totalDividendosFII,
        patrimonioFinal: plFinalFII,
      },
