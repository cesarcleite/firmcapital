...
async function carregarTaxasRegulatorias() {
  try {
    ...
    let tipoFundo = "FIP-IE"; // Padrão para FIP-IE
...
async function carregarTipoFII() {
  try {
    const codigo = (typeof TIPO_FUNDO_ATUAL !== 'undefined') ? TIPO_FUNDO_ATUAL : "FIP-IE";
    console.log("🔍 [FIP-IE] Carregando tipo de fundo com código:", codigo);
    const response = await api.getTiposFundos({ codigo: codigo });
    console.log("📡 [FIP-IE] Resposta getTiposFundos:", response);
    if (response.success && response.data.length > 0) {
      // CORREÇÃO: Buscar o tipo de fundo com o código EXATO, não apenas o primeiro
      tipoFundoFII = response.data.find(tf => tf.codigo === codigo);
      
      if (!tipoFundoFII) {
        // Fallback: se não encontrar o exato, pega o primeiro
        tipoFundoFII = response.data[0];
        console.warn("⚠️ [FIP-IE] Código exato não encontrado, usando primeiro:", tipoFundoFII.codigo);
      }
      
      console.log("✅ [FIP-IE] Tipo de fundo carregado:", {
        _id: tipoFundoFII._id,
        nome: tipoFundoFII.nome,
        codigo: tipoFundoFII.codigo
      });
    } else {
      console.error("❌ [FIP-IE] Nenhum tipo de fundo encontrado para código:", codigo);
    }
  } catch (error) {
    console.error("❌ [FIP-IE] Erro ao carregar tipo FII:", error);
  }
}
...
function setupCurrencyInputs() {
  ...
  percentInputs.forEach((input) => {
      // Force type text to allow commas
      input.type = "text";
...
function setupDependentFields() {
  ...
  // Lógica para Valor de Venda (2x Valor do Ativo)
  let valorVendaManuallyEdited = false;

  // Inicializar Valor de Venda como 2x Valor do Ativo se estiver vazio ou padrão
  const imovelInicial = parseCurrencyInput(valorImovel);
  if (imovelInicial > 0) {
      const vendaPadrao = imovelInicial * 2;
      valorVenda.value = vendaPadrao.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  }

  valorVenda.addEventListener("input", function() {
      valorVendaManuallyEdited = true;
  });
  ...
  valorImovel.addEventListener("input", function () {
    ...
    // Atualizar Valor de Venda se não foi editado manualmente
    if (!valorVendaManuallyEdited) {
        const novoValorImovel = parseCurrencyInput(this);
        const novoValorVenda = novoValorImovel * 2;
        valorVenda.value = novoValorVenda.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    }
  });
...
function calcular() {
  ...
  setTimeout(() => {
    try {
      ...
      // Atualizar interface com os resultados calculados
      // updateInterface já é chamada no final de performCalculation no fip-ie-calculos.js?
      // VERIFICADO: Sim, performCalculation chama updateInterface, updateCharts, updateTables, updateSensitivity.
      // ENTÃO O PROBLEMA NÃO É FALTA DE CHAMADA AQUI.
      
      // Se performCalculation chama tudo, então o problema é que performCalculation não está rodando ou updateInterface falha.
      // Mas se performCalculation falhasse, cairia no catch.
      
      // Vou manter o código original limpo, mas garantir que o display block aconteça.
...
function updateInterface(
          ...
          <div class="table-scroll">
            <table class="table" id="tabelaDetalhesDireto">
              <thead>
                ...
                <tr>
                  <th class="column-group group-patrimonio visible">Ativo/Projeto</th>
                  <th class="column-group group-patrimonio visible">Caixa</th>
                  <th class="column-group group-patrimonio visible">PL Início</th>
                  <th class="column-group group-receitas visible">Resultado Líquido Mensal</th>
                  ...
                  <th class="column-group group-custos visible">IR Receita</th>
              ...
              <thead>
                <tr>
                  ...
                  <th colspan="11" class="group-header" data-group="custos" onclick="toggleTableGroup('fii', 'custos')">
                    Custos FIP-IE <i class="fas fa-chevron-down"></i>
                  </th>
                ...
                <tr>
                  ...
                  <th class="column-group group-patrimonio visible">Caixa</th>
                  <th class="column-group group-patrimonio visible">PL Início</th>
                  <th class="column-group group-receitas visible">Fatur. Bruto</th>
                  <th class="column-group group-receitas visible">Vacância</th>
                  <th class="column-group group-receitas visible">Inadimpl.</th>
                  <th class="column-group group-receitas visible">Efetivo</th>
                  ...
                  <th class="column-group group-custos visible">ITBI</th>
                  <th class="column-group group-custos visible">IR Receit.</th>
      ...
      <div id="tab-custos" class="tab-content">
        <div class="chart-container">
          <h3 class="section-title">
            ...
            Composição dos Custos FIP-IE
          </h3>
          <div class="chart-wrapper">
            <canvas id="chartCustosComposicao"></canvas>
          </div>
        </div>
      </div>
            ...
            <table class="sensitivity-table" id="tabelaSensibilidade">
              <thead>
                <tr>
                  ...
                  <th class="text-right">Valor Total FIP-IE</th>
                  <th class="text-right">Diferença</th>
                  <th class="text-right">Diferença %</th>
                  <th>Resultado</th>
                </tr>
              </thead>
              <tbody id="tabelaSensibilidadeBody">
                <tr><td colspan="6" class="text-center">Em desenvolvimento...</td></tr>
              </tbody>
            </table>
          </div>
        </div>
  ...
  const diferencaElement = document.getElementById("diferencaAbsoluta");
  ...
  diferencaElement.className =
    diferenca >= 0 ? "metric-value positive" : "metric-value negative";

  if (Math.abs(diferenca) < 100) {
    diferencaTexto.textContent = "Equivalente";
    diferencaTexto.className = "metric-comparison";
  } else if (diferenca > 0) {
    diferencaTexto.textContent = "FIP-IE Vantajoso";
    diferencaTexto.className = "metric-comparison positive";
  } else {
    diferencaTexto.textContent = "Direto Vantajoso";
    diferencaTexto.className = "metric-comparison negative";
  }

  // Atualizar métricas
  updateMetric("tir", tirDiretoAnual, tirFIIAnual, true);
  updateMetric("roe", roeMedioDireto, roeMedioFII, true);
  ...
  vantagemElement.textContent = formatPercent(vantagem);
  if (Math.abs(vantagem) < 0.01) {
    vantagemElement.className = "metric-value";
    vantagemText.textContent = "Equivalente";
    vantagemText.className = "metric-comparison";
  } else if (vantagem > 0) {
    vantagemElement.className = "metric-value positive";
    vantagemText.textContent = "FIP-IE Vantajoso";
    vantagemText.className = "metric-comparison positive";
  } else {
...
function updateMetric(id, valDireto, valFII, isPercent = false) {
  const compElement = document.getElementById(`${id}Comparison`);
  const diffElement = document.getElementById(`${id}Diff`);

  const diff = valFII - valDireto;
  
  // Se isPercent for true, os valores já vêm como percentual (ex: 15.67 para 15,67%)
  // formatPercent divide por 100 internamente, então passamos o valor direto
  const formattedValDireto = isPercent ? formatPercent(valDireto) : valDireto;
  const formattedValFII = isPercent ? formatPercent(valFII) : valFII;
  const formattedDiff = isPercent ? formatPercent(Math.abs(diff)) : Math.abs(diff);

  compElement.textContent = `${formattedValDireto} vs ${formattedValFII}`;

  if (Math.abs(diff) < 0.01) {
    diffElement.textContent = "Equivalente";
    diffElement.className = "metric-comparison";
  } else if (diff > 0) {
    diffElement.textContent = `+${formattedDiff} (FIP-IE)`;
    diffElement.className = "metric-comparison positive";
  } else {
    diffElement.textContent = `-${formattedDiff} (Direto)`;
    diffElement.className = "metric-comparison negative";
  }
}
...
function updateCharts() {
        ...
        callbacks: {
          label: function (context) {
            ...
            if (context.parsed.y !== null) {
              if (
                context.chart.canvas.id === "chartROE" ||
                context.chart.canvas.id === "chartDividendYield"
              ) {
        ...
        datasets: [
          {
            ...
            data: dadosDireto.map((d) => d.plFinal + (d.dividendoAcumulado || 0)),
          ...
          {
            label: "Valor Total FIP-IE",
            data: dadosFII.map((d) => d.plFinal + (d.dividendoAcumulado || 0)),
            borderColor: COLORS.accentGold,
  ...
  // Gráfico de Composição dos Custos FIP-IE
...
function updateDetailTables() {
  const diretoBody = document.querySelector("#tabelaDetalhesDireto tbody");
  const fiiBody = document.querySelector("#tabelaDetalhesFII tbody");
    ...
    dadosDireto.forEach((d) => {
      ...
      diretoBody.innerHTML += `
        <tr ${rowClass}>
          ...
          <td class="column-group group-patrimonio visible">${formatCurrency(d.valorImovel)}</td>
          <td class="column-group group-patrimonio visible">${formatCurrency(d.valorCaixa)}</td>
          <td class="column-group group-patrimonio visible">${formatCurrency(d.plInicio || d.plFinal)}</td>
          <td class="column-group group-receitas visible">${formatCurrency(d.aluguelBruto)}</td>
          <td class="column-group group-receitas visible">${formatCurrency(d.perdaVacancia || 0)}</td>
          <td class="column-group group-receitas visible">${formatCurrency(d.perdaInadimplencia || 0)}</td>
          <td class="column-group group-receitas visible">${formatCurrency(d.aluguelEfetivo || (d.aluguelBruto - (d.perdaVacancia || 0) - (d.perdaInadimplencia || 0)))}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.custosOperacionais)}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.custoITBI)}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.irAluguel)}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.irVenda || 0)}</td>
          <td class="column-group group-custos visible custo-total">${formatCurrency(d.totalCustos || (d.custosOperacionais + d.custoITBI + d.irAluguel + (d.irVenda || 0)))}</td>
          <td class="column-group group-resultado visible ${lucroClass}">${formatCurrency(d.lucroOperacional)}</td>
          <td class="column-group group-resultado visible">${formatCurrency(d.irDividendo)}</td>
          <td class="column-group group-resultado visible">${formatCurrency(d.dividendo)}</td>
          <td class="column-group group-resultado visible">${formatCurrency(d.reinvestimento)}</td>
          <td class="column-group group-indicadores visible">${formatNumber(d.roe, 2)}%</td>
          <td class="column-group group-indicadores visible">${formatNumber(d.dy, 2)}%</td>
          <td class="column-group group-indicadores visible">${formatNumber(d.margem, 2)}%</td>
        </tr>
      `;
    });
  }
    ...
    dadosFII.forEach((d) => {
      ...
      fiiBody.innerHTML += `
        <tr ${rowClass}>
          ...
          <td class="column-group group-receitas visible">${formatCurrency(d.perdaInadimplencia || 0)}</td>
          <td class="column-group group-receitas visible">${formatCurrency(d.aluguelEfetivo || (d.aluguelBruto - (d.perdaVacancia || 0) - (d.perdaInadimplencia || 0)))}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.custosAdmin || 0)}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.custosGestao || 0)}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.custosCustodia || 0)}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.custosConsultoria || 0)}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.custoITBI)}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.irAluguel + (d.irVenda || 0))}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.taxaCVMAnualMes || 0)}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.taxaCVMRegistroMes || 0)}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.taxaANBIMARegistroMes || 0)}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.taxaDistribuicaoMes || 0)}</td>
          <td class="column-group group-custos visible">${formatCurrency(d.outrosCustosMes || 0)}</td>
          <td class="column-group group-custos visible custo-total">${formatCurrency(d.totalCustos || ((d.custosAdmin || 0) + (d.custosGestao || 0) + (d.custosCustodia || 0) + (d.custosConsultoria || 0) + d.custoITBI + d.irAluguel + (d.irVenda || 0) + (d.taxaCVMAnualMes || 0) + (d.taxaCVMRegistroMes || 0) + (d.taxaANBIMARegistroMes || 0) + (d.taxaDistribuicaoMes || 0) + (d.outrosCustosMes || 0)))}</td>
          <td class="column-group group-resultado visible ${lucroClass}">${formatCurrency(d.lucroOperacional)}</td>
          <td class="column-group group-resultado visible">${formatCurrency(d.irDividendo)}</td>
          <td class="column-group group-resultado visible">${formatCurrency(d.dividendo)}</td>
          <td class="column-group group-resultado visible">${formatCurrency(d.reinvestimento)}</td>
...
function updateSensitivity() {
  ...
  const totalDividendosDireto = dadosDireto.reduce((sum, d) => sum + d.dividendo, 0);
  ...
  scenarios.forEach((scenario) => {
    ...
    const diffPercent = Math.abs(scenario.direto) > 100 ? (diff / Math.abs(scenario.direto)) * 100 : 0;
    
    let resultado = "Equivalente";
    let resultadoClass = "neutral";
    
    if (Math.abs(diffPercent) >= 1) {
      if (diffPercent > 0) {
        resultado = "FIP-IE Melhor";
        resultadoClass = "success";
      } else {
        resultado = "Direto Melhor";
...
function mostrarAlertas() {
  ...
  if (mesesPrejuizoFII > 0) {
    alertas.push(`⚠️ FIP-IE: ${mesesPrejuizoFII} meses com prejuízo`);
  }
  ...
  if (plNegativoFII) {
    alertas.push(`🔴 FIP-IE: Patrimônio Líquido Negativo detectado`);
  }
...
async function gerarPDF() {
  if (typeof SimulacaoPDFGenerator === "undefined") {
    showNotification("Gerador de PDF não carregado", "error");
    return;
  }

  const generator = new SimulacaoPDFGenerator();
  await generator.gerarPDF();
}

async function gerarPDFSimulacao() {
...
async function salvarSimulacao(forcarNova = false) {
  try {
    ...
    // Verificar se tipoFundoFII está carregado
    if (!tipoFundoFII || !tipoFundoFII._id) {
        // Tentar recarregar ou usar um valor padrão se possível, ou alertar erro
        // Como fallback, tentar buscar do localStorage ou similar se houvesse, mas aqui vamos alertar
        console.warn("Tipo de fundo FIP-IE não carregado, tentando buscar...");
        // Se não tiver, o backend pode rejeitar. Vamos prosseguir mas logar.
    }
    ...
    if (simulacaoId && !forcarNova) {
      ...
      if (confirmed === null) return; // Cancelou (se fosse modal customizado que retorna null)
      // Nota: confirmAction retorna true/false. Se for false, cria nova.
      if (confirmed === false) {
        // Quer criar nova
        simulacaoId = null;
      }
    ...
    const dadosSimulacao = {
      ...
      descricao,
      cliente: clienteId,
      tipoFundo: tipoFundoFII ? tipoFundoFII._id : null, // Garantir que não quebre se nulo
      status: "concluida",
      parametros,
      resultados,
      detalhes: {
...
function handleError(error) {
  console.error(error);
  showNotification(error.message || "Ocorreu um erro", "error");
}
