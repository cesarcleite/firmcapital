/**
 * Gerador de PDF Premium para Simulação FIP-IE vs Investimento Direto
...
class SimulacaoPDFGenerator {
  ...
  criarCapa(doc) {
    ...
    doc.text(
      "Investimento Direto vs FIP-IE (Infraestrutura)",
      this.pageWidth / 2,
      88,
      { align: "center" }
    );
  ...
  criarResumoExecutivo(doc) {
    ...
    // Calcular métricas - USAR DADOS GLOBAIS SE DISPONÍVEIS
    let totalDivDireto, totalDivFII, plFinalDireto, plFinalFII, valorTotalDireto, valorTotalFII, diferenca, diferencaPct;

    if (window.resultadosSimulacao) {
      // Usar valores calculados pelo motor de simulação (mais preciso)
      const res = window.resultadosSimulacao;
      totalDivDireto = res.direto.totalDividendos;
      plFinalDireto = res.direto.plFinal;
      valorTotalDireto = res.direto.valorTotal;
      
      totalDivFII = res.fii.totalDividendos;
      plFinalFII = res.fii.plFinal;
      valorTotalFII = res.fii.valorTotal;
      
      diferenca = res.diferenca;
      diferencaPct = res.diferencaPct;
    } else {
      // Fallback (cálculo manual, pode ter imprecisões de arredondamento/correção)
      totalDivDireto = dadosDireto.reduce((sum, d) => sum + d.dividendo, 0);
      totalDivFII = dadosFII.reduce((sum, d) => sum + d.dividendo, 0);
      plFinalDireto = dadosDireto[dadosDireto.length - 1].plFinal;
      plFinalFII = dadosFII[dadosFII.length - 1].plFinal;
      valorTotalDireto = totalDivDireto + plFinalDireto;
      valorTotalFII = totalDivFII + plFinalFII;
      diferenca = valorTotalFII - valorTotalDireto;
      diferencaPct = (diferenca / valorTotalDireto) * 100;
    }
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...this.colors.primary);
    doc.text("FIP-IE (Infraestrutura)", col2X + cardWidth / 2, yPos + 8, {
      align: "center",
    });
    ...
    const vantagem = diferenca > 0 ? "FIP-IE" : "Direto";
    const corVantagem =
    ...
    doc.autoTable({
      startY: yPos,
      head: [["Métrica", "Inv. Direto", "FIP-IE", "Diferença"]],
    ...
    if (Math.abs(diferencaPct) < 1) {
      ...
      conclusao = `O Fundo de Investimento em Participações (FIP-IE) apresenta vantagem financeira de ${this.formatMoeda(
        diferenca
      ...
      conclusao = `O investimento direto demonstra vantagem de ${this.formatMoeda(
        ...
        2
      )}%) em relação ao FIP-IE. Esse resultado pode estar relacionado aos custos de administração do fundo e à estrutura tributária aplicada.`;
    }
  ...
  criarPremissas(doc) {
    ...
    const tributacaoDireto = [
      ["IR Rendimentos", document.getElementById("irAluguelDireto").value + "%"],
      [
        "IR Dividendos",
    ...
    const tributacaoFII = [
      ["IR Rendimentos", document.getElementById("irAluguelFII").value + "%"],
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...this.colors.primary);
    doc.text("FIP-IE (Infraestrutura)", this.pageWidth / 2 + 10, yPos - 5);
    ...
    // Seção 4: Custos FII
    this.adicionarSubtitulo(doc, "4. Custos e Taxas do FIP-IE", yPos);
    yPos += 8;
    ...
    const outrosParametros = [
      ...
      [
        "Distribuição Dividendos - FIP-IE",
  ...
  criarResultadosComparativos(doc) {
    ...
    let totalDivDireto, totalDivFII, plFinalDireto, plFinalFII, valorTotalDireto, valorTotalFII;
    ...
    let roeMedioDireto, roeMedioFII, dyMedioDireto, dyMedioFII, margemMedioDireto, margemMedioFII;

    if (window.resultadosSimulacao) {
      const res = window.resultadosSimulacao;
      roeMedioDireto = res.direto.roeMedio;
      dyMedioDireto = res.direto.dyMedio;
      margemMedioDireto = res.direto.margemMedia;
      
      roeMedioFII = res.fii.roeMedio;
      dyMedioFII = res.fii.dyMedio;
      margemMedioFII = res.fii.margemMedia;
    } else {
      roeMedioDireto = dadosDireto.reduce((sum, d) => sum + d.roe, 0) / dadosDireto.length;
      roeMedioFII = dadosFII.reduce((sum, d) => sum + d.roe, 0) / dadosFII.length;
      dyMedioDireto = dadosDireto.reduce((sum, d) => sum + d.dy, 0) / dadosDireto.length;
      dyMedioFII = dadosFII.reduce((sum, d) => sum + d.dy, 0) / dadosFII.length;
      margemMedioDireto = dadosDireto.reduce((sum, d) => sum + d.margem, 0) / dadosDireto.length;
      margemMedioFII = dadosFII.reduce((sum, d) => sum + d.margem, 0) / dadosFII.length;
    }
    ...
    if (custosFII && custosFII.length > 0) {
      ...
      doc.setFont("helvetica", "italic");
      doc.setTextColor(...this.colors.medium);
      const custoText = `Os custos do FIP-IE representam ${pctCustos.toFixed(
        2
  ...
  criarDetalhamentoMensal(doc) {
    ...
    const estatisticas = [
      ...
      ["Dividendo Médio - FIP-IE", this.formatMoeda(dividendoMedioFII)],
    ];
